...
def run(args):
    ...
    logger = get_logger(os.path.join('logs', 'MDD_{}.log'.format(timestr)))
    logger.info(args)
    model_path = 'saved_models/MDD_{}2{}_{}.pth'.format(args.src_data, args.tar_data, timestr)
    ...
    for tar_data in dataset:
        ...
        for r in range(args.repeat):
            ...
            # model definition
            model = MDD (n_feat, args.enc_hidden_dim, args.droprate,
            ...
            for batch_idx in range(args.epoch * n_batch):
                ...
                tr_loss = MDD_loss(s_out, t_out, s_labels, src_class_weight)
                tr_loss.backward()
                optimizer.step()
                ...
                if (batch_idx+1) % args.manipulate_batch == 0:
                    ...
                    mn_loss = MDD_loss(s_out, t_out, s_labels, src_class_weight)
