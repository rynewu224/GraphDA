...
def run(args):
    ...
    logger = get_logger(os.path.join('logs', 'DIVA_{}.log'.format(timestr)))
    logger.info(args)
    model_path = 'saved_models/DIVA_{}2{}_{}.pth'.format(args.src_data, args.tar_data, timestr)
    ...
    for tar_data in dataset:
        ...
        for r in range(args.repeat):
            ...
            for batch_idx in range(args.epoch * n_batch):
                ...
                src_tr_loss = DIVA_loss(s_out, s_labels, s_adj, 0, weights)
                tar_tr_loss = DIVA_loss(t_out, t_labels, t_adj, 1, weights)
                ...
                if (batch_idx+1) % args.manipulate_batch == 0:
                    ...
                    s_mn_loss = DIVA_loss(s_out, s_labels, s_nadj, 0, weights)
                    t_mn_loss = DIVA_loss(t_out, t_labels, t_nadj, 1, weights)
