...
def evaluate(model, loader, domain, device, verbose=True):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            y_score += out['sem_cls'][:, 1].data.tolist()
...
def run(args):
    ...
    logger = get_logger(os.path.join('logs', 'DSR_{}.log'.format(timestr)))
    logger.info(args)
    model_path = 'saved_models/DSR_{}2{}_{}.pth'.format(args.src_data, args.tar_data, timestr)
    ...
    for tar_data in dataset:
        ...
        for r in range(args.repeat):
            ...
            # model definition
            model = DSR (n_feat, args.enc_hidden_dim, args.droprate,
            ...
            for batch_idx in range(args.epoch * n_batch):
                ...
                src_tr_loss = DSR_loss(s_out, s_labels, s_adj, 0, weights)
                tar_tr_loss = DSR_loss(t_out, t_labels, t_adj, 1, weights)
                ...
                if (batch_idx+1) % args.manipulate_batch == 0:
                    ...
                    s_mn_loss = DSR_loss(s_out, s_labels, s_nadj, 0, weights)
                    t_mn_loss = DSR_loss(t_out, t_labels, t_nadj, 1, weights)
